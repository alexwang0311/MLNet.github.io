...
extern "C" struct gpt2_context * gpt2_init(const char * path_model) {
...
extern "C" void gpt2_free(struct gpt2_context * ctx) {
    delete ctx;
}

extern "C" const char * gpt2_get_prompt(struct gpt2_context * ctx) {
    return ctx->prompt_base.c_str();
}

extern "C" void gpt2_set_prompt(struct gpt2_context * ctx, const char * prompt) {
    ctx->prompt_base = prompt;
}

extern "C" std::vector<gpt_vocab::id> gpt2_tokenize(const gpt2_context * ctx, const char * text) {
    return ::gpt_tokenize(ctx->vocab, text);
}

extern "C" std::string gpt2_gen_text(gpt2_context * ctx, const char * text, int max_tokens) {
    int n_past = 0;
